...
function AchievementDot({ achievement, unlocked }) {
  ...
  const tooltip =
    ...
    createPortal(
      <div
        className="pointer-events-none fixed z-[9999] w-44 rounded-xl border border-eco-emerald-200 bg-white p-2.5 text-left shadow-xl"
        style={{
          left: coords.left,
          top: coords.top,
        ...
        <span className="flex items-center gap-1.5">
          <span
            className={`inline-block h-2.5 w-2.5 shrink-0 rounded-full ${
              unlocked ? colorClass : "bg-gray-300"
            }`}
          />
          <span className="text-xs font-bold text-eco-emerald-900">{achievement.title}</span>
        </span>
        <span className="mt-1 block text-[10px] leading-snug text-eco-emerald-700">
          {achievement.description}
        </span>
        {!unlocked && (
          <span className="mt-1.5 block text-[10px] font-semibold text-eco-emerald-500">
  ...
  return (
    <>
      ...
      >
        <span
          className={`inline-block h-4 w-4 rounded-full border-2 border-white shadow-sm sm:h-[18px] sm:w-[18px] ${
            unlocked ? colorClass : "bg-gray-200"
          }`}
  ...
  return (
    <div className="min-w-0 overflow-x-auto">
      <table className="w-full min-w-[640px] text-left text-base sm:min-w-[720px]">
        <thead className="bg-eco-emerald-50 text-xs uppercase tracking-wide text-eco-emerald-600 sm:text-sm">
          <tr>
        ...
        <tbody className="text-eco-emerald-900">
          {rows.map((row, index) => {
            ...
            return (
              ...
              >
                ...
                <td className="hidden px-4 py-3.5 text-sm text-eco-emerald-700 sm:table-cell sm:px-5 sm:py-4 sm:text-base">
