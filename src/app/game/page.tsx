...
export default function GamePage() {
  ...
  if (!isConfigured) {
    return (
      <div className="min-h-screen bg-eco-emerald-50 px-3 py-6 sm:px-6 sm:py-8">
        <div className="mx-auto max-w-3xl rounded-3xl border border-amber-200 bg-amber-50/90 p-6 text-center text-sm text-amber-900 shadow-sm sm:p-8">
  ...
  if (loading || !session) {
    return (
      <div className="min-h-screen bg-eco-emerald-50 px-3 py-6 sm:px-6 sm:py-8">
        <div className="mx-auto flex max-w-3xl flex-col items-center justify-center rounded-3xl border border-eco-emerald-200 bg-white/90 p-6 text-center shadow-sm sm:p-10">
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#d1fae5_0%,_#ecfdf5_44%,_#f7fee7_100%)] px-3 py-6 sm:px-6 sm:py-8">
      <div className="mx-auto max-w-6xl">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 sm:mb-6">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-eco-emerald-600">EcoURP</p>
            <h1 className="text-2xl font-black text-eco-emerald-950 sm:text-4xl">Eco-Catch</h1>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <AnimatedButton
              type="button"
              onClick={toggleFullscreen}
              className="rounded-full border border-eco-emerald-300 bg-white px-3 py-2 text-sm font-semibold text-eco-emerald-800 shadow-sm sm:px-4"
        ...
        <div ref={viewportRef} className={`relative ${isFullscreen ? "flex flex-col bg-gradient-to-b from-eco-emerald-900 to-eco-emerald-950" : ""}`}>
              ...
              {state.mode === "zen" && (
                <div className="absolute top-2 right-2 z-40 sm:top-3 sm:right-14">
                  <AnimatedButton
                    ...
                    className="flex items-center gap-1.5 rounded-full border border-rose-300/60 bg-rose-50/90 px-3 py-1.5 text-xs font-semibold text-rose-700 shadow-md backdrop-blur-sm sm:px-4 sm:py-2 sm:text-sm"
                  >
          ...
          {state.phase === "menu" ? (
            ...
            >
              <div className="w-full max-w-3xl max-h-[82dvh] overflow-y-auto rounded-3xl border border-white/80 bg-white/88 p-5 shadow-2xl shadow-eco-emerald-900/15 backdrop-blur-md sm:max-h-none sm:overflow-visible sm:p-8">
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-eco-emerald-600">Selecciona modo</p>
                <h2 className="mt-2 text-2xl font-black text-eco-emerald-950 sm:text-3xl">Atrapa y clasifica residuos</h2>
                <p className="mt-2 text-xs text-eco-emerald-700 sm:text-sm">
                  ...
                  {GAME_MODES.map((mode) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="mt-2 text-xs text-eco-emerald-800 sm:text-sm">{mode.description}</p>
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <AnimatedButton
                    ...
                    className="w-full rounded-2xl bg-eco-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md sm:py-3"
                  >
                    Iniciar partida
                  </AnimatedButton>
                  <Link
                    href="/ranking"
                    className="inline-flex w-full items-center justify-center rounded-2xl border border-eco-emerald-300 bg-white px-5 py-2.5 text-sm font-semibold text-eco-emerald-800 sm:py-3"
                  >
                    Ver ranking
                  </Link>
                </div>
