...
// ── Page ─────────────────────────────────────────────────────────────────────
export default function EcoCatchPage() {
  ...
  const canTutorialPrev =
    ...
    !(
      state.phase === "playing" &&
      tutorialSteps[tutorialStepIndex - 1]?.phase === "menu"
    );
  ...
  const tutorialFeedback = useMemo(
    () => buildWrongBinFeedback("plastic", "paper"),
    [],
  );
  ...
  useEffect(() => {
    if (!loading && isConfigured && !session) {
      router.replace("/login?next=/game/eco-catch");
    }
  ...
  useEffect(() => {
    const shouldBlockSpawns =
      (isTutorialOpen || isCountdownActive) && state.phase === "playing";
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 sm:mb-6">
          <div>
            ...
            <h1 className="text-2xl font-black text-foreground sm:text-4xl">
              Eco-Catch
            </h1>
          </div>
        ...
        >
          ...
          <TutorialCountdownOverlay
            open={isCountdownActive}
